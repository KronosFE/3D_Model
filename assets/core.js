...
window.KL=(function(){
"use strict";
const BASE=(()=>{try{const s=document.querySelector('script[src*="core.js"]');return new URL(s.getAttribute('src'),location.href).href.replace(/assets\/core\.js.*$/,'');}catch(e){return'';}})();
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(!document.querySelector('link[rel="icon"]')){
    const l=document.createElement('link');l.rel='icon';l.type='image/png';l.href=BASE+'assets/img/favicon.png';document.head.appendChild(l);
    const a=document.createElement('link');a.rel='apple-touch-icon';a.href=BASE+'assets/img/favicon_180.png';document.head.appendChild(a);
  }
  ...
  if(foot){
    ...
    foot.innerHTML=
      '<div class="footinner">'+
        '<div class="fbrand"><img class="femblem" src="'+BASE+'assets/img/kfe_emblem.png" alt=""><span><b>KRONOS</b> FUSION ENERGY · Kronos Model</span></div>'+
        '<div class="flinks">'+
          ...
          '<a href="'+BASE+'physics/deposits.html">The deposits</a>'+
        '</div>'+
...
async function loadEngine(){
  ...
  for(const f of ['kronos_clean.py','dt_evaluator.py']){
    const t=await (await fetch(BASE+'assets/engine/'+f,{cache:'no-cache'})).text();
    py.FS.writeFile(f,t);
  }
  py.runPython("import sys\nif '.' not in sys.path: sys.path.insert(0,'.')");
  const src=await (await fetch(BASE+'assets/engine/kronos_engine.py',{cache:'no-cache'})).text();
  py.runPython(src);
  _py=py;return py;
}
  ...
  try{
    ...
    if(allok){
      ...
      msg.innerHTML='<span class="ok-t">engine verified against the deposits</span> — '+
        ...
        '<a href="'+BASE+'physics/deposits.html">about the deposits →</a>';
    }else{
