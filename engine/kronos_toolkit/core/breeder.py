...
def _ion_mix(fuel, ne, f_he4=0.0, f_he3_ash=0.0):
    n_he4 = f_he4 * ne
    n_he3 = f_he3_ash * ne
...
def _reaction_rates(fuel, mix, T_keV):
    nD, nT = mix["nD"], mix["nT"]
...
def evaluate_breeder(fuel, R0, A, kappa, B0, q95, fG, Ti0, TBR_dt, TBR_dd=1.0,
    ...
    mix = _ion_mix(fuel, ne_p, f_he4=f_he4)
    Zeff_p = mix["Zeff"]
    Zeff = vol_avg(Zeff_p)

    R_DDp, R_DDn, R_DT, R_D3He = _reaction_rates(fuel, mix, Tp)
